...
public class SparkKafkaStreamingPrecipitationMain {
    ...
    private static final String Topic="precipitation";
        ...
        void start() throws Exception {
            SparkConf conf = new SparkConf().setAppName(APP_NAME).setMaster("local");
                ...
                stream.foreachRDD(rdd ->
                {
                    ...
                    JavaRDD<PrecipitationAndGeoZoneData> windRdd = rdd.map(new PrecipitationStationDataMapper());
                    // Map WindSpeedAndGeoZoneData objects by GeoZone

                    JavaPairRDD<String, AvgPrecipitationData> precipitationByZoneRdd = windRdd.mapToPair(d -> new Tuple2<>(d.getDeviceId(), new AvgPrecipitationData(d.getPrecipitation())));                    
// Reduce all data volume by GeoZone key
                    precipitationByZoneRdd = precipitationByZoneRdd.reduceByKey((a, b) -> AvgPrecipitationData.sum(a, b));
                    // Map <GeoZone, AvgWindSpeedData> back to WindSpeedAndGeoZoneData
                    List<PrecipitationAndGeoZoneData> aggData = precipitationByZoneRdd.map(t -> new PrecipitationAndGeoZoneData(t._1, t._2.getAvgValue(),t._2.getCount())).collect();                    
// Push aggregated data to ThingsBoard Asset
        ...
        private static class PrecipitationStationDataMapper implements Function<ConsumerRecord<String, String>, PrecipitationAndGeoZoneData> {
            private static final ObjectMapper mapper = new ObjectMapper();

            @Override
            public PrecipitationAndGeoZoneData call(ConsumerRecord<String, String> record) throws Exception {
                return mapper.readValue(record.value(), PrecipitationAndGeoZoneData.class);
            }
        }
    }
}